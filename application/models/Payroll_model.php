<?php
    ...
    public function GetDepDateEmployee($depid, $dt){
      $sql = "SELECT `employee`.*,
        `emp_salary`.`total`
        FROM `employee`
...
public function getPinFromID($employeeID){
    ...
    {
      $sql = "SELECT IFNULL(TRUNCATE((SUM(ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time`, `signout_time` ) ) )))/3600), 1), 0) AS Hours FROM `attendance` WHERE (`attendance`.`emp_id`='$employeePIN') AND (`atten_date` BETWEEN '$start_date' AND '$end_date')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
